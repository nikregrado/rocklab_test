...
$("#modal-board").on("submit", ".js-board-create-form", function() {
  var form = $(this);
  $.ajax({
    ...
    success: function(data) {
      if (data.form_is_valid) {
        $("#board_table tbody").html(data.html_board_list);
        $("#modal-board").modal("hide"); // <-- This is just a placeholder for now for testing
      } else {
        $("#modal-board .modal-content").html(data.html_form);
      }
    }
...
$(function() {
  ...
  // Update book
  $("#board_table").on("click", ".js-update-board", loadForm);
  $("#modal-board").on("submit", ".js-board-update-form", saveForm);

  $("#board_table").on("click", ".js-delete-board", loadForm);
